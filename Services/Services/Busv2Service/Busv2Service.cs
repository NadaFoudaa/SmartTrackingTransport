...
namespace Services.Services.Busv2Service
{
	public class Busv2Service : IBusv2Service
    {
		...
		{
			var busv2Repo = _unitOfWork.Repository<Bus>() as IBusv2Repository;
			if (busv2Repo == null)
				throw new InvalidOperationException("Bus repository not found in UnitOfWork");

			var Bus = await busv2Repo.GetAvailableBusesAsync(origin, destination);
			return _mapper.Map<IEnumerable<Busv2Dto>>(Bus);
		}
		...
		{
            var bus = await _unitOfWork.Busv2Repository.GetFirstOrDefaultIncludingAsync(
				b => b.Id == busId,
				b => b.Route.RouteStops.Select(rs => rs.Stop),
				b => b.BusTrips.Select(bt => bt.Trip),
				b => b.TrackingData
			);
